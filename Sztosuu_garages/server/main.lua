...
RegisterServerEvent('hunter_garages:removeCarFromParking')
AddEventHandler('hunter_garages:removeCarFromParking', function(plate, networkid)
	local xPlayer = ESX.GetPlayerFromId(source)
	if plate ~= nil then
		MySQL.Async.execute(
...
RegisterCommand('givecar', function(source, args, user)
	...
	if xPlayer.group == 'superadmin' or xPlayer.group == 'best' or xPlayer.group == 'admin' then
		if args[2] ~= nil then
			...
			givecarwebhook('FlashlifeRP.PL', wiadomosc, 11750815)
		else
...
RegisterCommand('delcar', function(source, args, user)
	...
	if xPlayer.group == 'superadmin' or xPlayer.group == 'best' or xPlayer.group == 'admin' then
		...
		delcarwebhook('FlashlifeRP.PL', wiadomosc, 11750815)
	end
end, false)
...
AddEventHandler('garages:savecarlog', function() 
	...
	savecarwebhook('FlashlifeRP.PL', wiadomosc, 11750815)
end)

function savecarwebhook(hook,message,color)
    local savecarwebhook = ''
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
